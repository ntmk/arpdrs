...
namespace HoloToolkit.Sharing.Utilities
{
    ...
    public class ManualIpConfiguration : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ipAddress.text = PlayerPrefs.GetString("SharingServerIp", "192.168.0.102");
            firstRun = true;
        }
        ...
        private void Update()
        {
            if (timerRunning && timer - Time.time < 0)
            {
                if (isTryingToConnect)
                {
                    ...
                    String message = "\nConnection Failed. No Sharing Service Found.";
                    NetworkFeedback.text = HelperFunctions.ResolveTextSize(message);

                    String message2 = "\n\nPlease Try Again.";
                    NetworkFeedback.text += HelperFunctions.ResolveTextSize(message2);
                }
            }
        }
